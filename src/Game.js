...
import { boxColorToTargetColor } from '../utils/ColorUtils.js'
import {Direction} from '../consts/Direction.js'
import {offsetForDirection} from '../utils/TileUtils.js'
...
export default class Game extends Phaser.Scene
{
	...
	create()
	{
		...
		player = layer.createFromTiles(52, 0, { key: 'tiles', frame: 52 }).pop();
		player.setOrigin(0);
	...
	extractBoxes(layer)
	{
		...
		boxColors.forEach(color => {
			boxesByColor[color] = layer.createFromTiles(color, 0, { key: 'tiles', frame: color}).map(box => box.setOrigin(0))
		})
	...
	tweenMove(direction, baseTween, onStart)
	{
		...
		if (boxData) {
			...
			if (nextBoxData) console.log(true + " " + nextWallData);
			else console.log(false + " " + nextWallData)
			...
			if (coveredTarget) {
				this.changeTargerCoveredCountForCount(targetColor, -1)
			}
	...
	changeTargerCoveredCountForCount(color, change)
	{
